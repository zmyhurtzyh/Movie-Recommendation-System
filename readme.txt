本项目由成都理工大学宜宾校区的三位同学在学习借鉴其他项目后完成的电影推荐系统的数据库项目。三位同学分别为曾铭杨，杨皓麟，陈禧锦。
...
电影推荐系统运行说明书
安装运行方法
1.解压项目，用pycharm打开manage.py。
2.在pycharm配置python解释器，(python的版本3.6.x-3.7)。
3.打开终端输入pip install -r requirements.txt 若提示无pip。去下载get-pip.py 运行python get-pip.py
4.安装成功后，使用pip list 查看已经安装好的package，和requirements.txt中的进行对比，确认安装无误。
有两种运行方法:
1.通过终端运行: python manage.py runserver
2.通过Pycharm配置运行，右上角选中movie，一键运行，第一次运行时间可能比较长，因为需要将item_recommend的相似矩阵计算出来。将模型保存到本地。
技术介绍：
